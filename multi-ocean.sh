...
services:
  ocean-node:
    ...
    environment:
      ...
      RPCS: '{
  "1": {
    "rpc": "https://lb.drpc.org/ogrpc?network=ethereum&dkey=AmeGw2GuvEyfkvVeBGz02JjQyUPYiJAR76rJOpXEh2H0",
    "chainId": 1,
    "network": "mainnet",
    "chunkSize": 100
  },
  "10": {
    "rpc": "https://lb.drpc.org/ogrpc?network=optimism&dkey=AmeGw2GuvEyfkvVeBGz02JjQyUPYiJAR76rJOpXEh2H0",
    "chainId": 10,
    "network": "optimism",
    "chunkSize": 100
  },
  "137": {
    "rpc": "https://lb.drpc.org/ogrpc?network=polygon&dkey=AmeGw2GuvEyfkvVeBGz02JjQyUPYiJAR76rJOpXEh2H0",
    "chainId": 137,
    "network": "polygon",
    "chunkSize": 100
  },
  ...
  "11155111": {
    "rpc": "https://lb.drpc.org/ogrpc?network=sepolia&dkey=AmeGw2GuvEyfkvVeBGz02JjQyUPYiJAR76rJOpXEh2H0",
    "chainId": 11155111,
    "network": "sepolia",
    "chunkSize": 100
  },
  "11155420": {
    "rpc": "https://lb.drpc.org/ogrpc?network=optimism-sepolia&dkey=AmeGw2GuvEyfkvVeBGz02JjQyUPYiJAR76rJOpXEh2H0",
    "chainId": 11155420,
...
menu() {
  ...
  echo "6) Автоматическая проверка состояния"
...
# Функция для проверки доступности нод каждые N времени, перезагружает только недоступные ноды
toggle_monitoring() {

функция работает вкл/выкл
проверяем работает ли функция
если да — выключить да/нет (выключаем обратно в меню/ просто обратно в меню)
если нет — включить да/нет (переходим к включению/обратно в меню)
читаем из валета сколько нод
Сколько интервал проверки
токен бота тг
функция работает (обратно в меню)
}
...
update_containers() {
  ...
  for dir in "$SCRIPT_DIR"/ocean*; do
    if [ -d "$dir" ]; then
      cd "$dir"
      docker compose up -d
      echo "Контейнер в директории $dir запущен. Ждем 3 секунд перед запуском следующего."
      sleep 3
      cd "$SCRIPT_DIR"
    fi
  done
...
# Функция для перезагрузки всех контейнеров с 3-секундной паузой
restart_containers() {
  clear
  echo "Перезагружаем все контейнеры с паузой в 3 секунд..."
  for dir in "$SCRIPT_DIR"/ocean*; do
    if [ -d "$dir" ]; then
      cd "$dir"
      ...
      docker compose down
      docker compose up -d
      echo "Контейнеры в директории $dir перезапущены. Ждем 3 секунд перед следующим."
      sleep 3
      cd "$SCRIPT_DIR"
    fi
  done
